...
\vspace{0.5 cm}
\uncover<2->{Only late-stage HEP analysis fits this model, not AOD.}
...
\begin{frame}[fragile]{Alternative: FIXME(add arxiv reference)}
\vspace{0.5 cm}
\textcolor{darkorange}{\fbox{\begin{minipage}{\linewidth}
\bf Instead of deserializing data into objects, translate the code into loops over raw arrays.
\end{minipage}}}
...
\begin{frame}{Performance on real queries}
\vspace{0.5 cm}
Four sample queries (see backup) using \textcolor{results4}{4.~ROOT with BulkIO and transformed Python code} (copied from last page), \textcolor{results5}{5.~raw arrays on SSD disk}, \textcolor{results6}{6.~raw arrays in memory}.
