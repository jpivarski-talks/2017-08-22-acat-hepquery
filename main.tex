...
\vspace{0.25 cm}
\textcolor{darkblue}{\normalsize into indexes of ``eventoffset'', ``jetoffset'', ``jetptdata'' arrays:}
...
\vspace{0.25 cm}
Disk reads and RAM strides are more sequential.

\vspace{0.25 cm}
Only columns used in the code are actually loaded: an object's attributes only exist if they're used.
\end{frame}
...
\begin{frame}{How it fits together}
...
\vspace{0.5 cm}
The translated Python can then be compiled by Numba\footnotemark[2] (Python compiler for numerical algorithms) into fast bytecode.

\vspace{0.5 cm}
Our contributions to ROOT\footnotemark[3] allow ROOT TBranch data to be rapidly viewed as Numpy arrays.

\vspace{0.5 cm}
This is being gathered into a HEPQuery application\footnotemark[4] to provide a query service. See README for more complex examples.
