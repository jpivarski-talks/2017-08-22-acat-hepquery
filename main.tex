...
\vspace{0.25 cm}
\textcolor{darkblue}{\normalsize \underline{into indexes over ``events'', ``jets'', ``jetpt'' arrays:}}
...
\vspace{1 cm}
Object-oriented calculations can be performed at \textcolor{red}{$\mathcal{O}(\mbox{10--100 MHz})$}.
...
\vspace{0.5 cm}
\uncover<3->{We may not be able to find exactly what we want on the open-source market, but we can find most of the pieces.}
\end{frame}
...
Femtocode is a mini-language based on Python, but with sufficient limitations to allow SQL-like query planning.
